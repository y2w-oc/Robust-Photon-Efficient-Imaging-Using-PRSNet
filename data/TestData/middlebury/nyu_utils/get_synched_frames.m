...
%depthfiledir=fullfile(sceneDir,files{ll});
    
    %depthfile=regexp(ls(files), '\s+|\n', 'split');
    %depthfile(end)=[];
    num_file=size(files,2);
    for ii=1:num_file
        if strfind(files{ii},'d-')
            numDepth = numDepth + 1;
            frameList(numDepth).rawDepthFilename = files{ii};
        end
        if strfind(files{ii},'a-')
            numAccel = numAccel + 1;
            accelRecs(numAccel).name = files{ii};
        end
        if strfind(files{ii},'r-')
            numRgb = numRgb+1;
  ...
  % if ~isempty(regexp(files{ii}, '^r-*', 'once'))
      % numRgb = numRgb + 1;
      % rgbImages(numRgb).name = files{ii};
